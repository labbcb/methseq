...
workflow PicoMethyl {
    ...
    output {
        ...
        File unmapped_qc_report_1 = UnmappedQC.report
        File unmapped_qc_report_zip_1 = UnmappedQC.reportZip
...
task Align {
    ...
    output {
        File bam = basename(fastq, ".fq.gz") + "_bismark_bt2.bam"
        File stats = basename(fastq, ".fq.gz") + "_bismark_bt2_SE_report.txt"
        File nucleotide_coverage = basename(fastq, ".fq.gz") + "_bismark_bt2.nucleotide_stats.txt"
        File unmapped = basename(fastq) + "_unmapped_reads.fq.gz"
    }
